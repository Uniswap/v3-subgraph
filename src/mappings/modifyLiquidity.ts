...
import { Bundle, Pool, PoolManager, Tick, Token } from '../types/schema'
...
import { ModifyLiquidity } from '../types/Factory/Pool'

export function handleModifyLiquidity(event: ModifyLiquidity): void {
  handleModifyLiquidityHelper(event)
}

export function handleModifyLiquidityHelper(event: ModifyLiquidity, subgraphConfig: SubgraphConfig = getSubgraphConfig()): void {
  const factoryAddress = subgraphConfig.factoryAddress

  const bundle = Bundle.load('1')!
  const poolId = event.params.id.toHexString()
  const pool = Pool.load(poolId)!
  const factory = PoolManager.load(factoryAddress)!
  ...
  if (token0 && token1) {
    ...
    if (
      ...
      BigInt.fromI32(event.params.tickUpper).gt(pool.tick as BigInt)
        ...
        if(event.params.liquidityDelta.ge( BigInt.fromI32(0) )) {
          pool.liquidity = pool.liquidity.plus(event.params.liquidityDelta)
        } else {
          pool.liquidity = pool.liquidity.minus(event.params.liquidityDelta)
        }
    }
    ...
    if(event.params.liquidityDelta.ge( BigInt.fromI32(0) )) {
      ...
      upperTick.liquidityNet = upperTick.liquidityNet.minus(amount) // bug? minus?
    } else {
      lowerTick.liquidityGross = lowerTick.liquidityGross.minus(amount)
      upperTick.liquidityGross = upperTick.liquidityGross.minus(amount)
      lowerTick.liquidityNet = lowerTick.liquidityNet.minus(amount)
      upperTick.liquidityNet = upperTick.liquidityNet.plus(amount) // bug? 
    }
